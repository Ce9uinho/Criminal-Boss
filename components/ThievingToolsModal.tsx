...
export default function ThievingToolsModal({ visible, onClose }: Props) {
  const { thievingToolsOwned, equippedThievingToolId, acquireThievingTool, equipThievingTool, bank, gold } = useGameStore();

  const tools = useMemo(() => THIEVING_TOOLS.slice().sort((a, b) => a.tier - b.tier), []);
  const activeTool = useMemo(() => THIEVING_TOOLS.find(t => t.id === equippedThievingToolId), [equippedThievingToolId]);

  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose} testID="tools-modal">
        <View style={styles.modal}>
          <View style={styles.header}>
            <View style={styles.titleRow}>
              <Wrench size={16} color="#E0B252" />
              <Text style={styles.title}>Heist Gear</Text>
            </View>
            <View style={styles.headerRight}>
              <TouchableOpacity onPress={onClose} style={styles.closeBtn} testID="tools-close">
                <X size={18} color="#BDB5AA" />
              </TouchableOpacity>
            </View>
          </View>

          <Text style={styles.helper}>Acquire gear using loot and cash. Equip one to boost your success.</Text>

          <ScrollView style={styles.list} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}>
            {tools.map(tool => {
              const owned = !!thievingToolsOwned?.[tool.id];
              const equipped = equippedThievingToolId === tool.id;
              ...
              return (
                <View key={tool.id} style={styles.card} testID={`tool-${tool.id}`}>
                  ...
                  <View style={styles.bonusesRow}>
                    <Text style={styles.bonus}>- {(100 - Math.round(tool.bonuses.failureReductionMultiplier * 100))}% Catch Rate</Text>
                    <Text style={styles.bonus}>- {(100 - Math.round(tool.bonuses.cooldownReductionMultiplier * 100))}% Cooldowns on Caught</Text>
                    {tool.bonuses.extraLootChance ? (
                      <Text style={styles.bonus}>+{Math.round(tool.bonuses.extraLootChance * 100)}% chance to get +1 loot</Text>
                    ) : (
                      <Text style={styles.bonus}>+ {Math.round(((tool.bonuses.lootBonusMultiplier ?? 1) - 1) * 100)}% Extra Loot Chance</Text>
                    )}
                    <Text style={styles.bonus}>- {Math.max(0, (100 - Math.round(tool.bonuses.heatReductionMultiplier * 100)))}% Heat Gained</Text>
                  </View>
