...
export default function SmugglingToolsModal({ visible, onClose }: Props) {
  const { smugglingToolsOwned, equippedSmugglingToolId, acquireSmugglingTool, equipSmugglingTool, bank, gold } = useGameStore();

  const tools = useMemo(() => SMUGGLING_TOOLS.slice().sort((a, b) => a.tier - b.tier), []);

  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <TouchableOpacity style={styles.overlay} activeOpacity={1} onPress={onClose} testID="smuggling-tools-modal">
        <View style={styles.modal}>
          <View style={styles.header}>
            <View style={styles.titleRow}>
              <Wrench size={16} color="#E0B252" />
              <Text style={styles.title}>Smuggling Crews</Text>
            </View>
            <View style={styles.headerRight}>
              <TouchableOpacity onPress={onClose} style={styles.closeBtn} testID="smuggling-tools-close">
                <X size={18} color="#BDB5AA" />
              </TouchableOpacity>
            </View>
          </View>

          <Text style={styles.helper}>Recruit crews using smuggling loot and cash. Equip one to reduce time, cut junk, and improve rare finds.</Text>

          <ScrollView style={styles.list} contentContainerStyle={styles.listContent} showsVerticalScrollIndicator={false}>
            {tools.map(tool => {
              const owned = !!smugglingToolsOwned?.[tool.id];
              const equipped = equippedSmugglingToolId === tool.id;
              ...
              return (
                <View key={tool.id} style={styles.card} testID={`smuggling-tool-${tool.id}`}>
                  ...
                  <View style={styles.bonusesRow}>
                    ...
                    <Text style={styles.bonus}>- {(100 - Math.round((tool.bonuses.junkReductionMultiplier ?? 1) * 100))}% Junk</Text>
                    {typeof tool.bonuses.itemsPerActionBonus === 'number' && tool.bonuses.itemsPerActionBonus > 0 && (
                      <Text style={styles.bonus}>+ {tool.bonuses.itemsPerActionBonus} Items</Text>
                    )}
                  </View>
